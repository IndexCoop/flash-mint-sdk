...
const indexToken = getTokenByChainAndSymbol(chainId, 'DPI')
...
describe('ZeroExTransactionBuilder()', () => {
  ...
  test('returns a tx for minting DPI (ERC20)', async () => {
    const buildRequest = createBuildRequest()
  ...
  test('returns a tx for minting DPI (ETH)', async () => {
  ...
  test('returns a tx for redeeming DPI (ERC20)', async () => {
  ...
  test('returns a tx for redeeming DPI (ETH)', async () => {
